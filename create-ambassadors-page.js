...
/**
 * Script to create the Ambassadors page in the correct Next.js App Router structure
 */
...
console.log('🚀 Setting up Ambassadors Page\n');

// Create ambassadors directory
createDir('app/ambassadors');

// Create the ambassadors page content
const ambassadorsPageContent = `"use client";
...
export default function AmbassadorsPage() {
  ...
  const ambassadors = [
    ...
    },
    {
      ...
      expertise: ["Community Engagement", "Workshop Facilitation", "Public Speaking"],
      events: 28,
      rating: 4.8,
      image: "AA",
      contact: {
    ...
    },
    {
      ...
      expertise: ["Youth Programs", "Social Media Literacy", "Content Creation"],
      events: 31,
      rating: 4.7,
      image: "MM",
      contact: {
  ...
  const filteredAmbassadors = ambassadors.filter((ambassador) => {
    const matchesSearch = ambassador.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      ambassador.expertise.some(exp => exp.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center space-y-4 mb-12">
          ...
          <p className="text-xl text-gray-200 max-w-3xl mx-auto">
            Connect with our passionate media literacy educators and advocates from around the world
          </p>
        </div>
        ...
        <div className="mb-8 space-y-4">
          <div className="max-w-2xl mx-auto">
            <input
              ...
              className="w-full px-6 py-3 rounded-full bg-purple-800/30 border border-purple-400/30 text-white placeholder-gray-400 focus:outline-none focus:border-yellow-400/50 focus:bg-purple-800/40 transition-all"
            />
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAmbassadors.map((ambassador) => (
            <div
              key={ambassador.id}
              className="bg-gradient-to-br from-purple-800/40 to-blue-800/40 rounded-xl p-6 backdrop-blur-sm border border-purple-400/20 hover:border-yellow-400/30 transition-all"
            >
              {/* Profile Header */}
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center text-xl font-bold text-black">
                    {ambassador.image}
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-300">{ambassador.title}</p>
                ...
                <span className={\`px-2 py-1 text-xs rounded-full \${
                  ...
                  {ambassador.availability === "Available for events" ? "🟢" :
                   ambassador.availability === "Limited availability" ? "🟡" : "💻"} Available
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {ambassador.expertise.map((exp, index) => (
                  <span
                    key={index}
                    className="px-2 py-1 text-xs bg-purple-900/50 text-purple-300 rounded-full border border-purple-400/30"
                  >
                    {exp}
                  </span>
                ))}
              ...
              {/* Stats */}
              <div className="flex justify-between mb-4 pb-4 border-b border-purple-400/20">
              ...
              <div className="flex gap-3">
                {Object.entries(ambassador.socials).map(([platform, url]) => (
                  <a
                    key={platform}
                    href={url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="p-2 bg-purple-900/50 rounded-lg text-gray-300 hover:text-yellow-400 hover:bg-purple-900/70 transition-all"
                    title={platform}
                  >
              ...
              {/* Contact Button */}
              <button className="w-full mt-4 px-4 py-2 bg-gradient-to-r from-yellow-400 to-orange-500 text-black font-semibold rounded-lg hover:from-yellow-300 hover:to-orange-400 transition-all">
                Schedule a Meeting
              </button>
            </div>
          ))}
...
// Write the ambassadors page
writeFile('app/ambassadors/page.tsx', ambassadorsPageContent);

// Delete the old file if it exists
if (fs.existsSync('app/ambassadors.tsx')) {
  fs.unlinkSync('app/ambassadors.tsx');
  console.log('✓ Removed old ambassadors.tsx file');
}

console.log('\n✅ Ambassadors page setup complete!');
console.log('The page should now be accessible at http://localhost:3000/ambassadors');
